...
// Upgrade NOTE By codecat: replaced '_Object2World' with 'unity_ObjectToWorld'

//
// Original: Unity 5.2.1f1 built in shader "Skybox/Procedural"
// Modified by n-yoda
//
Shader "codecat/Skybox/Starry-sky" {
	Properties{
		[Header(Sun)]
	...
	[Toggle] _Stars("Stars", Int) = 1
		...
		_StarsAlpha("Stars Alpha", Range(0.0,1.0)) = 1.0
	}
...
#define SKYBOX_COLOR_IN_TARGET_COLOR_SPACE 0
	...
	half4 frag(v2f IN) : SV_Target
	{
		...
		if (y < 0.0)
		{
			...
			half hash = dot(_StarsHash.xyz, center / 1.2) % _StarsHash.w;
